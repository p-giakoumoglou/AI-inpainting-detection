...
class IID_Model(nn.Module):
    ...
    def process(self, Ii, Mg):
        ...
        assert Mo.min() >= 0 and Mo.max() <= 1, "Labels must be in [0, 1]"
    ...
    def load(self, path=''):
        state_dict = torch.load(self.save_dir + path + 'IID_weights.pth')
...
def load_model(model, path):
    ...
    # Adjust for the case where the model was trained with DataParallel, which
    # prefixes the keys with 'module.'.
    new_state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
    model.load_state_dict(new_state_dict)

#model = IID_Net().cuda()  # Ensure this matches your model's definition.
